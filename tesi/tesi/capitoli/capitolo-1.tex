...
\chapter{Tools and applications}
	...
	\section{Radio Propagation Model}
		...
		\begin{itemize}
			...
			\item \textbf{Deterministic} path loss models, such as Friis propagation model, which models quadratic path loss as it occurs in free space, and Two Ray Ground, which assume propagation via two rays: a direct (LOS) one, and the one reflected by the ground.
	...
	\section{Obstacle Shadowing propagation loss model}
		The original thesis \cite{ROM2017} used a deterministic \gls{rpma} called Obstacle Shadowing propagation loss model presented in \cite{5720204} and implemented by the authors of \cite{Carpenter:2015:OMI:2756509.2756512}.  This propagation model calculates the loss in signal strength due to the shadowing effect of obstacles such as buildings. 
		
		
		Other RPMs do not keep into conside
