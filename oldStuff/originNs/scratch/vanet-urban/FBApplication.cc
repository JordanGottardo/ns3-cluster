...
//	NS_LOG_INFO (this << count);
	...
	if (count > 0)
	{
		...
		{
			if (i >= hel - 1) {
				NS_LOG_INFO("GenerateHelloTraffic dentro for i = ");
				NS_LOG_INFO(i);
			}
...
//	NS_LOG_UNCOND("m_nNodes = " << m_nNodes);
...
//				NS_LOG_UNCOND("hop = " << current->GetNum();
